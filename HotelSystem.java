...
public class HotelSystem {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case "3":
                    ...
                    return;
                default:
                    System.out.println("エラー: 無効な選択です。1, 2, 3のいずれかを入力してください。");
            }
        }
    }
    ...
    private static void handleCheckInCheckOut(Scanner scanner, RoomManagementScreen roomUI, RoomReservationProcess proc) {
        ...
        try {
            ...
            System.out.println("エラー: 予約番号は数字で入力してください。");
            return;
        }
        ...
        if (res != null) {
            ...
            // 【修正】変数名を 'cRoom' に修正し、チェックイン・チェックアウトのロジックを修正
            Room targetRoom = res.getRoom();

            // 部屋が使用中（チェックイン済み）かどうかで処理を分岐
            if (targetRoom.isInUse()) {
                // チェックイン済みの場合 -> チェックアウト処理へ
